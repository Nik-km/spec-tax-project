...
library(olsrr)          # Variable selection process
library(stargazer)
library(kableExtra)
library(magrittr)
library(readr)
...
cat('"', vec, sep='", "')   # easily copy & paste vector items    # cat("'", vec, sep="', '")
cat('"', vec, sep = '", \n  "')
cat(vec, sep=' + ')
cat(vec, sep=', ')
...
stargazer(did.base, 
          ...
          single.row = TRUE,                 # compress table output
