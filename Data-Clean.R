...
# Check weird unicode characters... Montréal
...
Inc.dist <- Inc.dist %>% 
    ...
    dplyr::mutate(Statistics = fct_recode(Statistics,
                                          "under_10k" = "Percentage under $10,000 (including zeros and losses)", 
                                          "10k_19k" = "$10,000 to $19,999", 
                                          "20k_29k" = "$20,000 to $29,999",
                                          "30k_39k" = "$30,000 to $39,999",
                                          "40k_49k" = "$40,000 to $49,999", 
                                          "50k_59k" = "$50,000 to $59,999",
                                          "60k_79k" = "$60,000 to $79,999",
                                          "60k_69k" = "$60,000 to $69,999", 
                                          "70k_79k" = "$70,000 to $79,999",
                                          "80k_99k" = "$80,000 to $99,999", 
                                          "80k_89k" = "$80,000 to $89,999",
                                          "90k_99k" = "$90,000 to $99,999",
                                          "over_100k" = "$100,000 and over", 
                                          "100k_149k" = "$100,000 to 149,999",
                                          "over_150k" = "$150,000 and over")) %>% 
    dplyr::filter(Statistics %in% c("under_10k", "10k_19k", "20k_29k", "30k_39k", "40k_49k",
                                    "50k_59k", "60k_69k", "70k_79k", "80k_89k", "90k_99k",
                                    "100k_149k", "over_100k", "over_150k")) %>%
    dplyr::select(Date, GEO, val_norm, Statistics) %>% 
    droplevels() %>% 
    tidyr::spread(Statistics, val_norm, fill = NA)

Inc.dist <- Inc.dist %>%  
    dplyr::mutate(`10k_39k` = `10k_19k` + `20k_29k` + `30k_39k`,  # Constructing larger income brackets
                  `40k_59k` = `40k_49k` + `50k_59k`,
                  `60k_79k` = `60k_69k` + `70k_79k`,
                  `80k_99k` = `80k_89k` + `90k_99k`) %>%
    dplyr::select(Date, GEO, `under_10k`, `10k_39k`, `40k_59k`, `60k_79k`, `80k_99k`, `100k_149k`, `over_100k`, `over_150k`)
...
pop.kid <- Census %>% 
    dplyr::filter(Statistics == "Number of families",
                  ...
                  Family_type %in% c("Family types without children",
                                     ...
                                     "Family types with 3 or more children")) %>% 
    dplyr::mutate(Family_type = fct_recode(Family_type,
                                           "pop_noKid" = "Family types without children",
                                           "pop_kid1" = "Family types with 1 child",
                                           "pop_kid2" = "Family types with 2 children",
                                           "pop_kid3" = "Family types with 3 or more children")) %>% 
    dplyr::select(Date, GEO, val_norm, Family_type) %>% 
    droplevels() %>% 
    tidyr::spread(Family_type, val_norm, fill = NA)
...
Inc.cpl <- Census %>%
    ...
    dplyr::mutate(Family_type = fct_recode(Family_type,
                                           ...
                                           "Inc_cpl_wk" = "Family types with 1 child",
                                           "Inc_cpl_w2k" = "Family types with 2 children",
                                           "Inc_cpl_w3k" = "Family types with 3 or more children")) %>% 
    dplyr::select(Date, GEO, val_norm, Family_type) %>%
    droplevels() %>%
    tidyr::spread(Family_type, val_norm, fill = NA) %>% 
    dplyr::mutate(Inc_cpl_wkids = (Inc_cpl_w2k + Inc_cpl_w3k)/2) %>% # combining & averaging the income of couples w/ 2 or more kids
    dplyr::select(Date, GEO, Inc_allcpl, Inc_cpl_nk, Inc_cpl_wk, Inc_cpl_wkids)
...
# Data to interpolate: 
    # Inc.cpl, Inc.med, pop.age, pop.cpl, pop.fam, pop.kid
...
df_Inc.cpl <- df_Inc.cpl %>% 
    ...
    map(. %>%
            dplyr::mutate(Inc_allcpl = round(na.spline(object = Inc_allcpl, na.rm = FALSE), 0),
                          ...
                          Inc_cpl_wk = round(na.spline(object = Inc_cpl_wk, na.rm = FALSE), 0),
                          Inc_cpl_wkids = round(na.spline(object = Inc_cpl_wkids, na.rm = FALSE), 0))
    ) %>% 
...
df_pop.kid <- df_pop.kid %>% 
    group_by(GEO, .add = TRUE) %>% group_split() %>% 
    map(. %>%
            dplyr::mutate(pop_noKid = round(na.spline(object = pop_noKid, na.rm = FALSE), 0),
                          pop_kid1 = round(na.spline(object = pop_kid1, na.rm = FALSE), 0),
                          pop_kid2 = round(na.spline(object = pop_kid2, na.rm = FALSE), 0),
                          pop_kid3 = round(na.spline(object = pop_kid3, na.rm = FALSE), 0))
    ) %>% 
    data.table::rbindlist()     # merge the list of regions
...
# last minute changes to aggregate groupings
Inc.cpl <- df_Inc.cpl %>% 
    dplyr::rename(Inc_cpl_child = Inc_cpl_wk,
                  Inc_cpl_kids = Inc_cpl_wkids) %>% 
    dplyr::select(Date, GEO, Inc_allcpl, Inc_cpl_nk, Inc_cpl_child, Inc_cpl_kids)

pop.kid <- df_pop.kid %>%   
    dplyr::mutate(pop_wkids = pop_kid2 + pop_kid3) %>% 
    dplyr::select(Date, GEO, pop_noKid, pop_kid1, pop_wkids)


rm(df.groups, df_Inc.cpl, df_Inc.med, df_pop.age, df_pop.cpl, df_pop.fam, df_pop.kid)
...
write.csv(df.full, "~\\Projects\\spec-tax-project\\Data Cleaning\\DATA_CANSIM.csv", row.names = FALSE)
write.csv(df.full, "~\\Projects\\spec-tax-project\\FULL-DATA.csv", row.names = FALSE)
